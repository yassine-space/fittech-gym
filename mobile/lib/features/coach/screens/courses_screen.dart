...
class _CoachCoursesScreenState extends State<CoachCoursesScreen>
          ...
          body: SafeArea(
            child: Column(
              ...
              children: [
                ...
                _buildStats(allMy, provider.coursesLoading, provider.coursesError),
                _buildLevelFilter(),
                _buildTabs(upcoming, ended),
                Expanded(child: _buildBody(provider, upcoming, ended)),
              ],
            ),
          ),
  ...
  Widget _buildStats(List<Course> allMy, bool loading, String? error) {
    ...
    final totalEnrolled = allMy.fold<int>(0, (sum, c) => sum + (c.maxParticipants - c.spotsRemaining));

    return Padding(
      padding: const EdgeInsets.fromLTRB(24, 16, 24, 0),
      child: Row(
        children: [
          _StatCard(label: 'Upcoming', value: '$upcomingCount', color: _kOrange),
          const SizedBox(width: 10),
          _StatCard(label: 'Ended', value: '$endedCount', color: _kGrey),
          const SizedBox(width: 10),
          _StatCard(label: 'Enrolled', value: '$totalEnrolled', color: _kGreen),
        ],
      ),
    );
  }
  ...
  Widget _buildBody(CoachProvider provider, List<Course> upcoming, List<Course> ended) {
    if (provider.coursesLoading && provider.courses.isEmpty) {
      return Center(child: CircularProgressIndicator(color: _kOrange));
    }
    if (provider.coursesError != null && provider.courses.isEmpty) return _buildError(provider);
    return TabBarView(
      controller: _tabController,
      children: [
    ...
    if (courses.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(
              isEnded ? Icons.history_rounded : Icons.upcoming_rounded,
              size: 56,
              color: _kOrange.withOpacity(0.3),
            ),
            ...
            Text(
              ...
              style: TextStyle(
                  color: _kGrey, fontWeight: FontWeight.w600, fontSize: 15),
            ),
            if (!isEnded) ...[
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Connection error',
              style: TextStyle(
                  color: _kNavy, fontWeight: FontWeight.w700, fontSize: 16)),
...
class _CoachCourseCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final enrolled = course.maxParticipants - course.spotsRemaining;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isEnded ? Colors.white.withOpacity(0.7) : Colors.white,
        borderRadius: BorderRadius.circular(20),
        border: isEnded
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: LinearGradient(
                colors: isEnded
                    ? [_kGrey.withOpacity(0.4), _kGrey.withOpacity(0.2)]
                    : [_kOrange, _kNavy],
              ),
            ),
          ),
                ...
                Column(
                  ...
                  children: [
                    ...
                    if (!isEnded && course.spotsRemaining > 0)
                      Text('${course.spotsRemaining} spots remaining',
                          style:
                              TextStyle(fontSize: 11, color: _kGrey)),
                    if (isEnded)
                      Text(
                        enrolled > 0
            ...
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.add_rounded, color: _kOrange, size: 22),
                ),
